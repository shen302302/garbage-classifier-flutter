...
//    }
    ...
    init {
        ...
        out1 = outputShape[1] // e.g., 4 + num_classes
        out2 = outputShape[2] // e.g., 8400
        Log.d("TAG", "Model output shape = [1, $out1, $out2]")

        // Fallback: if labels couldn't be loaded from model metadata, derive class count
        // from the output tensor shape (YOLO outputs [x,y,w,h, conf_0, conf_1, ...])
        if (labels.isEmpty()) {
            val inferredNumClasses = out1 - 4
            if (inferredNumClasses > 0) {
                labels = (0 until inferredNumClasses).map { "class_$it" }.toList()
                Log.w(TAG, "Labels not found in metadata. Inferred $inferredNumClasses classes from output shape.")
            } else {
                Log.e(TAG, "Cannot infer numClasses: out1=$out1, expected > 4")
            }
        }
