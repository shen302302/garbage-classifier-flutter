...
object YOLOFileUtils {
    ...
    fun loadLabelsFromAppendedZip(context: Context, modelPath: String): List<String>? {
        ...
        for (currentPathAttempt in pathsToTry) {
            ...
            // Flutter's resolveModelPath already prepends "flutter_assets/" for asset paths.
            // AssetManager.openFd needs this prefix intact because Flutter assets in the APK
            // are stored at "assets/flutter_assets/..." (not "assets/assets/...").
            // If the path lacks the prefix (legacy), add it so AssetManager can find the file.
            if (!resolvedPath.startsWith("flutter_assets/") && !resolvedPath.startsWith("/")) {
                resolvedPath = "flutter_assets/$resolvedPath"
            }
