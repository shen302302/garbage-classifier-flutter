...
class YOLOPlugin : FlutterPlugin, ActivityAware, MethodChannel.MethodCallHandler, PluginRegistry.RequestPermissionsResultListener {
   ...
   * Flutter asset paths (e.g. "assets/models/foo.tflite") are stored in the APK
   * under "flutter_assets/" prefix, so we must prepend it for native asset loading.
   * @param modelPath The model path from Flutter
   * @return Resolved absolute path or Android asset path with flutter_assets/ prefix
   */
  private fun resolveModelPath(modelPath: String): String {
    ...
    // Flutter asset path → resolve to actual Android asset path in APK
    // Flutter stores assets under "flutter_assets/" inside Android's assets directory
