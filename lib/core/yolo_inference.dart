...
class YOLOInference {
  ...
  Future<Map<String, dynamic>> predict(
    ...
    logger.i('Starting YOLO inference for task: ${_task.name}');
    final stopwatch = Stopwatch()..start();

    if (imageBytes.isEmpty) {
      logger.e('Image data is empty', error: 'Empty image bytes');
      throw InvalidInputException('Image data is empty');
    }

    if (confidenceThreshold != null &&
        (confidenceThreshold < 0.0 || confidenceThreshold > 1.0)) {
      logger.w('Invalid confidence threshold: $confidenceThreshold');
      throw InvalidInputException(
        'Confidence threshold must be between 0.0 and 1.0',
      );
    }
    if (iouThreshold != null && (iouThreshold < 0.0 || iouThreshold > 1.0)) {
      logger.w('Invalid IoU threshold: $iouThreshold');
    ...
    try {
      ...
      if (confidenceThreshold != null) {
        ...
        logger.i('Using confidence threshold: $confidenceThreshold');
      }
      if (iouThreshold != null) {
        arguments['iouThreshold'] = iouThreshold;
        logger.i('Using IoU threshold: $iouThreshold');
      }

      if (_instanceId != 'default') {
        arguments['instanceId'] = _instanceId;
        logger.i('Using instance ID: $_instanceId');
      }

      logger.d('Sending inference request with ${imageBytes.length} bytes');
      ...
      stopwatch.stop();
      logger.i('Inference completed in ${stopwatch.elapsedMilliseconds}ms');

      if (result is Map) {
        final processedResult = _processInferenceResult(result);
        logger.d('Processed result: ${processedResult.length} items');
        return processedResult;
      }

      logger.e('Invalid result format returned from inference');
      throw InferenceException('Invalid result format returned from inference');
    } on PlatformException catch (e) {
      logger.e('PlatformException during inference', error: e);
      throw YOLOErrorHandler.handleError(e, 'Error during image prediction');
    } catch (e) {
      logger.e('Error during YOLO inference', error: e, stackTrace: StackTrace.current);
      throw YOLOErrorHandler.handleError(e, 'Error during image prediction');
    }
  }
