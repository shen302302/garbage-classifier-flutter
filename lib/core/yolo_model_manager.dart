...
class YOLOModelManager {
  ...
  Future<void> initializeInstance() async {
    logger.i('Initializing YOLO instance: $_instanceId');
    try {
      if (_instanceId != 'default') {
        logger.d('Creating YOLO instance with ID: $_instanceId');
      ...
      _isInitialized = true;
      logger.i('YOLO instance initialized successfully');
    } catch (e) {
      logger.e('Failed to initialize YOLO instance: $_instanceId', error: e);
  ...
  Future<bool> loadModel() async {
    logger.i('Loading model: $_modelPath for task: ${_task.name}');
    if (!_isInitialized) {
      logger.d('Instance not initialized, initializing now...');
      await initializeInstance();
    }
    ...
    try {
      ...
      logger.d('Sending load model request with arguments: $arguments');
      final result = await _channel.invokeMethod('loadModel', arguments);
      final success = result == true;

      if (success) {
        logger.i('Model loaded successfully: $_modelPath');
      } else {
        logger.e('Failed to load model: $_modelPath');
      }

      return success;
    } on PlatformException catch (e) {
      logger.e('Failed to load model: $_modelPath', error: e);
      throw YOLOErrorHandler.handleError(
        e,
